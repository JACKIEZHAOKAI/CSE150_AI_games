...
# helper funciton for MC
def simulation_sequence( state, userCard, dealCard, ccards):
    episode = []
    userSum = state[0]
    userA = state[1]
    dealSum = state[2]
    ...
    while True:
        ...
        #play the game and create new state
...
# helper funciton for TD  and   Q learning 
def simulate_one_step( state, userCard, dealCard, ccards, Q_Learning, action):
    ...
    # if game over, calc reward and return 
    if gameover:       
        if userSum == dealSum:
            reward = 0
        elif userSum <= 21 and len(userCard) == 5:
    ...
    # if not game over, play the game to calc next state
    if not Q_Learning:
        action = policy(userSum)
...
def reward_to_go(s, gamma, episode):
    ...
    # episode is  a list of tuple (state, reward)
    reward = 0
    i=0 
    k=0
    flag = False 
...
def MC_Policy_Evaluation(policy, states, gamma, MCvalues, G):
    ...
    for simulation in range(50):
        ...
        state = make_state(userSum, userA, dealFirst, dealAFirst)
        # do simulation
        episode = simulation_sequence( state, userCard, dealCard, ccards)
        # update   
        for e in episode:
            #This line is a placeholder. Remove. Need more lines too, of course. 
            currState = e[0]
            # API reward_to_go(s, gamma, episode):
            G[currState].append(reward_to_go(currState, gamma, episode))
            MCvalues[currState] = np.mean(G[currState])
...
def TD_Policy_Evaluation(policy, states, gamma, TDvalues, NTD):
    ...
    for simulation in range(50):
        ...
        while state is not None:
            ...
            NTD[state] += 1
            # API  simulate_one_step( state, userCard, dealCard, ccards, Q_Learning, action):
            next_state,reward = simulate_one_step( state,userCard,dealCard,ccards, False, 0)
            alpha = 10/(9+NTD[state])

            # calculate TDvalues 
            if next_state is None:
                TDvalues[state] = TDvalues[state] + alpha*(reward + 0 - TDvalues[state])
            else:
                TDvalues[state] = TDvalues[state] + alpha*(reward + gamma*TDvalues[next_state] - TDvalues[state])

            state = next_state


# helper function for Q learning 
def pick_action(state, eps, Qvalues):
    # while in computation budget
    # eps should be positive and very small
    ## Random float x, 0.0 <= x < 1.0
    if random.random() < eps:
        # take rand action 0 or 1
...
def Q_Learning(states, gamma, Qvalues, NQ):
    ...
    for simulation in range(50):
        ...
        while state is not None:
            ...
            #API simulate_one_step( state, userCard, dealCard, ccards, Q_Learning, action)::
            next_state, reward=  simulate_one_step(state, userCard, dealCard, ccards, True, action)

            # update Q value
            alpha = 10/(9+NQ[state])

            if next_state is not None:
                Qvalues[state][action] = Qvalues[state][action] + alpha * (reward + gamma * max( Qvalues[next_state][0], Qvalues[next_state][1] ) - Qvalues[state][action] )
            else:
                Qvalues[state][action] = Qvalues[state][action] + alpha * (reward + gamma * 0  - Qvalues[state][action] )
            state = next_state
