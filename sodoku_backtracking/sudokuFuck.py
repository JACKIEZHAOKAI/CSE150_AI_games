...
###################################################################
	def search(self, assignment, unassignedSpot):
		...
		domainLenList = [ (len(self.grid.domains[s]), s) for s in unassignedSpot if len(self.grid.domains[s]) >= 1 ]
		print ("unassignedSpot]", unassignedSpot)
		print("domainLenList", domainLenList)

		# select an unsigned spot with min domain from unassignedSpot list.
		n,spot = min( domainLenList )
		...
		for value in self.grid.domains[spot]:
			...
			if self.consistent(spot, value, assignment):
				...
				infrenceDict, inferSucc = self.infer(spot,value,assignment)
				...
				if inferSucc == True:
					...
					for infSpot, infVal in infrenceDict.items():
						...
						for peer in self.grid.peers[infSpot]:
							if infVal in self.grid.domains[peer]:
								self.grid.domains[peer].remove(infVal)
				...
				for peer in self.grid.peers[spot]:
					self.grid.domains[peer].append(value)
		return False
...
###################################################################
	# def infer(self, spot, value, assignment):
	...
	def infer(self, spot, guessValue, assignment):
		...
		print("########################")
		for peer in self.grid.peers[spot]:
			if guessValue in self.grid.domains[peer]:
				self.grid.domains[peer].remove(guessValue)
				...
				if len(self.grid.domains[peer]) == 1:
					inferVal = self.grid.domains[peer][0]
					if self.consistent(peer, inferVal, assignment) == False:
						inferSuccess = False
						break
					else:
		...
		if inferSuccess == False:   # add domain value back back
			for infSpot,infVal in infrenceDict.items():
				for peer in self.grid.peers[spot]:
					self.grid.domains[peer].append(value)
				return {},False
