...
class Board:
    ...
    def autoplay(self):
        ...
        if not self.game_over:
            start = time.time()
            player1 = Randplay(self.grid, self.piece)   # constructor 
            r,c = player1.make_move()                   # make a move
            print("rand Auto", self.piece, "move: (", r, ",", c, ")")
            end = time.time()
            print ("time cost of Rand auto is: ", end - start)
            self.set_piece(r, c)
            self.check_win(r, c)
        ...
        if not self.game_over:
            ...
            start = time.time()
            player2 = MCTS(self.grid, self.piece)       # constructor 
            r,c = player2.uct_search()                  # make a move
            print("AI", self.piece, "move: (", r, ",", c, ")")
            end = time.time()
            print ("time cost of AI is: ", end - start)
            self.set_piece(r, c)
            self.check_win(r, c)
    ...
    def semi_autoplay(self):
        if not self.game_over:
        #Optional: Change this to MCTS AI and see whether you can reward

            start = time.time()
            player1 = MCTS(self.grid, self.piece)       # constructor 
            r,c = player1.uct_search()                  # make a move            
            print("semi-AI", self.piece, "move: (", r, ",", c, ")")  
            end = time.time()
            print ("time cost of AI is: ", end - start)
            self.set_piece(r, c)
            self.check_win(r, c)  
